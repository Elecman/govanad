...
func (bks *BtcKeySet) GenKeySet() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "GenKeySet failed: %v", err)
	}
...
func (bks *BtcKeySet) GetPrivKey(format string) (string, error) {
	if bks.privKey == nil {
		return "", errors.New("GetPrivKey failed: the private key is nil")
	}
	switch format {
	case "decimal":
	...
	default:
		return "", fmt.Errorf("GetPrivKey failed: \"%s\" is not a supported format", format)
	}
}
...
func (bks *BtcKeySet) GetPubKey(format string) (string, error) {
	if bks.pubKey == nil {
		return "", errors.New("GetPubKey failed: the public key is nil")
	}
	switch format {
	case "hex":
	...
	default:
		return "", fmt.Errorf("GetPubKey failed: \"%s\" is not a supported format", format)
	}
}
...
func (bks *BtcKeySet) GetAddr(isCompressed bool) (string, error) {
	if bks.pubKey == nil {
		return "", errors.New("GetAddr failed: the public key is nil")
	}
...
func (bks *BtcKeySet) CreateQR(timestamp int64) {
	...
	if wifPrivKey, err := encrp.EncryptPrivKey(bks, false); err != nil {
		fmt.Fprintf(os.Stderr, "CreateQR failed: %v", err)
	} else {
...
func (bks *BtcKeySet) CreateTxt(timestamp int64) {
	// create text file
	txtName := fmt.Sprintf("%d_txt_keyaddr.txt", timestamp)
	f, err := os.Create(txtName)
	if err != nil {
		fmt.Fprintf(os.Stderr, "CreateTxt failed: %v", err)
		return
	}
	defer f.Close()
